...
def eval(test_loader, net):
    ...
    return [valid_losses.avg, valid_top1.avg, cur_EER_valid, cur_HTER_valid, auc_score, threshold, ACC_threshold, ace, tdr]

def re_eval(test_loader, path, save_number):
    net = Resnet18()
    model_dict = net.state_dict()
    pretrained_dict = torch.load(path + str(save_number) + '.pth')
